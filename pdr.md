...
## 5. Gestione Task
- **Riordino Task**: Le task ora possono essere riordinate tramite trascinamento (drag-and-drop) tenendole premute. È stata reintegrata la libreria SortableJS configurata con un ritardo ridotto (`delay: 300ms`) che attiva il trascinamento dopo la pressione prolungata, accompagnando la fine del delay con una vibrazione aptica immediata (sincronizzata tramite l'evento `onChoose`) per segnalare fisicamente l'aggancio dell'elemento. Funziona uniformemente sia con tocco che con mouse, permettendo la pacifica convivenza con lo swipe-to-delete e lo scroll verticale.
